...
export async function GET(req: NextRequest) {
  try {
    ...
    const results = users.map((u) => ({
      ...
      college: u.college ? `${u.college.collegeName} · ${u.college.branch} · ${u.college.yearOfPassing}` : null,
      workplace: u.workplace ? `${u.workplace.companyName} · ${u.workplace.city}` : null,
      gym: u.gym ? `${u.gym.gymName} · ${u.gym.city}` : null,
      neighbourhood: u.neighbourhood ? `${u.neighbourhood.premisesName} · ${u.neighbourhood.city}` : null,
    }));
