...
export default function AdminPaymentSettings({ initialCatalog, initialAppSettings }: AdminPaymentSettingsProps) {
  const [catalog, setCatalog] = useState(initialCatalog);
  const [drafts, setDrafts] = useState<DraftState>(() => buildDraftState(initialCatalog));
  const [savingService, setSavingService] = useState<string | null>(null);
  const [appSettings, setAppSettings] = useState(initialAppSettings);
  const [appSettingsDraft, setAppSettingsDraft] = useState(initialAppSettings);
  ...
  async function handleSaveAppSettings() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(getResponseErrorMessage(data, "Failed to update app settings"));
      }

      setAppSettings(data.appSettings as AppSettings);
      setAppSettingsDraft(data.appSettings as AppSettings);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="glass rounded-3xl p-5 sm:p-6">
        <div className="flex flex-col gap-2 mb-5">
          <h2 className="text-2xl font-semibold" style={{ color: "#3f2c1d" }}>iConfess Contact Settings</h2>
          <p className="text-sm" style={{ color: "#735a43" }}>
            Control the public Instagram ID, Snapchat ID, contact email, and feedback email used across the app.
          </p>
        </div>
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            {([
              { key: "instagramId", label: "Instagram ID", placeholder: "iconfess.in" },
              { key: "snapchatId", label: "Snapchat ID", placeholder: "iconfess" },
              { key: "contactEmail", label: "Contact Us Email", placeholder: "support@example.com" },
              { key: "feedbackEmail", label: "Feedback Email", placeholder: "feedback@example.com" },
            ] as const).map((field) => (
              <label key={field.key} className="block">
                ...
                <input
                  type="text"
                  value={appSettingsDraft[field.key]}
          ...
          <div className="mt-4 rounded-xl px-4 py-3 text-sm" style={{ background: "rgba(255,255,255,0.55)", color: "#735a43" }}>
            Current public values: Instagram `@{appSettings.instagramId || "NA"}`, Snapchat `@{appSettings.snapchatId || "NA"}`, contact `{appSettings.contactEmail}`, feedback `{appSettings.feedbackEmail}`.
          </div>
          ...
          <div className="mt-5 flex justify-end">
            ...
            >
              <Save className="h-4 w-4" />
              {savingAppSettings ? "Saving..." : "Save Contact Settings"}
            </button>
          </div>
        </div>
      </section>
